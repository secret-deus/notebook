...
Plan 模式深度集成在 [[how-claude-code-works/11-permission-security|权限系统]] 中。`prepareContextForPlanMode()` 的行为取决于进入前的模式：
...
Plan 模式为 [[how-claude-code-works/07-multi-agent|多 Agent 架构]] 中的子 Agent 提供专用指令（`src/utils/messages.ts:3399-3417`）。子 Agent 的计划文件使用独立的命名空间（`{slug}-agent-{agentId}.md`），避免与主会话的计划文件冲突。
...
上一章：[[how-claude-code-works/07-multi-agent|多 Agent 架构]] | 下一章：[[how-claude-code-works/05-code-editing-strategy|代码编辑策略]]
