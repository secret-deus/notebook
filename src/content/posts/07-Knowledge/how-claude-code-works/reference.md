...
| 概念 | 一句话解释 | 详见 |
|------|-----------|------|
| **Agent Loop** | 用户输入 → 模型决策 → 工具执行 → 结果注入的循环，直到模型返回纯文本 | [[how-claude-code-works/02-agent-loop|第 2 章]] |
...
| **search-and-replace** | FileEditTool 的编辑策略，要求 old_string 在文件中唯一匹配 | [[how-claude-code-works/05-code-editing-strategy|第 10 章]] |
| **纵深防御** | 7 层独立安全检查，任一层被绕过不致命 | [[how-claude-code-works/11-permission-security|第 12 章]] |
...
| **Hooks** | 事件驱动扩展机制，在工具执行生命周期的关键节点注入自定义逻辑 | [[how-claude-code-works/06-hooks-extensibility|第 7 章]] |
...
返回：[[how-claude-code-works/quick-start|快速入门]] | [首页](/)
